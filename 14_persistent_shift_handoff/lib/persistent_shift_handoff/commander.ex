defmodule PersistentShiftHandoff.Commander do
  @moduledoc false
  ...
  @impl true
  def init(_opts) do
    # Subscribe during init so alerts are not dropped during startup or restart.
    PersistentShiftHandoff.AlertBus.subscribe(:critical_alerts)
  ...
  def handle_info({:incident_alert, :critical_alerts, alert}, state) do
    ...
    :ok = PersistentShiftHandoff.MaintenanceQueue.enqueue(action)
